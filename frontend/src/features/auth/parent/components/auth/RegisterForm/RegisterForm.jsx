      ...
      >
        ...
        <div className="mb-5">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Join 10,000+ students learning Islamic studies
          </p>
        </div>

        {/* Form - Slightly more spacing */}
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          {formData.password && (
            ...
            >
              <div className="flex gap-1">
                <div className={`h-1 flex-1 rounded-full ${formData.password.length >= 6 ? 'bg-emerald-500' : 'bg-slate-200'}`}></div>
                <div className={`h-1 flex-1 rounded-full ${formData.password.length >= 8 ? 'bg-emerald-500' : 'bg-slate-200'}`}></div>
                <div className={`h-1 flex-1 rounded-full ${formData.password.length >= 10 ? 'bg-emerald-500' : 'bg-slate-200'}`}></div>
                <div className={`h-1 flex-1 rounded-full ${formData.password.match(/[A-Z]/) && formData.password.match(/[0-9]/) ? 'bg-emerald-500' : 'bg-slate-200'}`}></div>
              </div>
              <p className="text-[10px] text-slate-400">
                {formData.password.length < 6 && "Use at least 6 characters"}
                {formData.password.length >= 6 && formData.password.length < 8 && "Add more characters"}
                {formData.password.length >= 8 && !formData.password.match(/[A-Z]/) && "Add uppercase letters"}
                {formData.password.length >= 8 && formData.password.match(/[A-Z]/) && !formData.password.match(/[0-9]/) && "Add numbers"}
                {formData.password.length >= 8 && formData.password.match(/[A-Z]/) && formData.password.match(/[0-9]/) && "✓ Strong password"}
              </p>
            </motion.div>
          )}
          ...
          >
            <div className="relative mt-0.5">
              ...
              >
                {termsAccepted && <FaCheckCircle className="text-white text-[8px]" />}
              </div>
            </div>
            <label className="text-[10px] text-slate-500 leading-relaxed cursor-pointer">
              I agree to the{' '}
              <Link to="/terms" className="text-emerald-600 hover:text-emerald-800 font-medium">
                Terms
              </Link>
              {' '}and{' '}
          ...
          <motion.button
            ...
            className={`w-full py-2.5 text-sm bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-medium transition-all duration-300 ${
              termsAccepted 
                ? "hover:shadow-lg hover:shadow-emerald-500/30 transform hover:-translate-y-0.5" 
        ...
        >
          <div className="flex items-center justify-center gap-3 text-[9px] text-slate-400">
            ...
            <span className="w-px h-3 bg-slate-300"></span>
            <span className="flex items-center gap-1">
              <FaGraduationCap className="text-emerald-500 text-[8px]" />
              Free Access
            </span>
          </div>
        </motion.div>
